...
const fetchVolatilityFileMetrics = async (req, res, next) => {
    try {
        ...
        const {
            filter_by_provider: filterByProvider, filter_by_country: filterByCountry,
        ...
        for (let i = 0; i < uniqueList.length; i++) {
            ...
            const data = await LoadLogModel.findAll({
                attributes: [
                    [Sequelize.fn('CONVERT', Sequelize.literal('DATE'), Sequelize.col('LOADSTARTTIME')), 'date'],
                    [sequelize.fn('COUNT', sequelize.col('*')), 'count'],
                ],
                group: [
                    Sequelize.fn('CONVERT', Sequelize.literal('DATE'), Sequelize.col('LOADSTARTTIME')),
                ],
                where: {
                    Source: uniqueList[i],
                    ...whereClause
